...
//static int sleep[SLEEPY_NDEVICES];
...
//A condition function using to chech flag, lock and unlock
	     ...
	     loff_t *f_pos)
{
  ...
  if(count > 4 || count == 0){
     printk("Please input 4-byte integer.\n");
     mutex_unlock(&dev->sleepy_mutex);
     return -EINVAL;
  }
  ...
  mutex_unlock(&dev->sleepy_mutex);
  interrupt = wait_event_interruptible_timeout(dev->wq, dev->flag != 0,write*HZ);
